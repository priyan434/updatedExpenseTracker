...
exports.register = async (req, res) => {
  ...
  try {
    ...
    if (error) {
      ...
      const errorResponse = errorMap[errorDetails.path[0]]?.[errorDetails.type] || errorMap[errorDetails.path[0]]?.["default"] || INVALID_INPUT;
...
exports.updateProfile = async (req, res) => {
  ...
  try {
    ...
    const profile = {
      ...
      profileUrl: profileData.profileUrl.toString('base64')
    };
...
if(!decodedData.email){
  ...
  try {
    ...
    const { error, value } = validationSchema.validate({ password,confirmPassword });
    if (error) {
      const errorDetails = error.details[0];
      const errorMap = {
